...
class ChairModel(ObjectronModel):
    model_path: str = "mediapipe/modules/objectron/" "object_detection_3d_chair.tflite"
    label_name: str = "Chair"


@attr.s(auto_attribs=True, frozen=True)
class CameraModel(ObjectronModel):
    model_path: str = "mediapipe/modules/objectron/" "object_detection_3d_camera.tflite"
    label_name: str = "Camera"


@attr.s(auto_attribs=True, frozen=True)
class CupModel(ObjectronModel):
    model_path: str = "mediapipe/modules/objectron/" "object_detection_3d_cup.tflite"
...
class Objectron(SolutionBase):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            calculator_params={
                ...
                (
                    "boxlandmarksubgraph__ThresholdingCalculator" ".threshold"
                ): min_tracking_confidence,
                ("Lift2DFrameAnnotationTo3DCalculator" ".normalized_focal_x"): fx,
                ("Lift2DFrameAnnotationTo3DCalculator" ".normalized_focal_y"): fy,
                (
                    "Lift2DFrameAnnotationTo3DCalculator"
                    ".normalized_principal_point_x"
                ): px,
                (
                    "Lift2DFrameAnnotationTo3DCalculator"
                    ".normalized_principal_point_y"
                ): py,
            },
    ...
    def _convert_format(
        ...
        for annotation in inputs.annotations:
            ...
            new_outputs.append(
                ObjectronOutputs(
                    landmarks_2d, landmarks_3d, rotation, translation, scale=scale
                )
            )
        return new_outputs
